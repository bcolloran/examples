...
nb.isMatrix = function (obj, rowsToCheck = 100) {
  ...
  for (let i = 0; i < rowsToCheck; i++) {
    if (!_.isArray(obj[i]) || obj.length == 0) {
      return false;
    }
    if (!obj[0].length === obj[i].length) {
      return false;
    }
  }
  return true;
...
nb.isColumnDf = function (obj, colsToCheck = 100) {
  ...
  for (let i = 1; i < colsToCheck; i++) {
    if (obj[cols[i]].length != numRows) return false;
  }
  return true;
};
...
nb.dropRowsWithNaN = function (col1, col2) {
  return _.unzip(
    _.zip(col1, col2).filter((xy) => !(_.isNaN(xy[0]) || _.isNaN(xy[1])))
  );
};
